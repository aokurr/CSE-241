...
namespace ahmet_adapter{
	...
	template <template <typename K,class Alloc = allocator<K>> class T>
	HexAdapter<T>::HexAdapter(int size_){
	}
	template <template <typename K,class Alloc = allocator<K>> class T>
	void HexAdapter<T>::play_game(){
		int m;
		char a;
		int i,j;
		bool flag=true;
	...
	template <template <typename K,class Alloc = allocator<K>> class T>
	void HexAdapter<T>::creat_empty_array(){
		int i,j;
		mt19937 generator;
	...
	template <template <typename K,class Alloc = allocator<K>> class T>
	void HexAdapter<T>::play(char column,int row){
		if(turn==0){
			while(1){
				try{
	...
	template <template <typename K,class Alloc = allocator<K>> class T>
	int HexAdapter<T>::finish_control_x(){
		int i;
		int flag=0;	
		int counter=1;
	...
	template <template <typename K,class Alloc = allocator<K>> class T>
	int HexAdapter<T>::recursion_x(int column,int row,int counter){
	...
	template <template <typename K,class Alloc = allocator<K>> class T>
	int HexAdapter<T>:: finish_control_o(){
		int i,counter=0;
		int flag=0;
	...
	/*FİNİSH CONTROL*/
	template <template <typename K,class Alloc = allocator<K>> class T>
	int HexAdapter<T>:: recursion_o(int column,int row,int counter){
		/*north*/
	...
	template <template <typename K,class Alloc = allocator<K>> class T>
	int HexAdapter<T>::save_load(const string &str){		
		/*SAVING PROCEDURES*/
		char laod_control;
	...
	template <template <typename K,class Alloc = allocator<K>> class T>
	void HexAdapter<T>::writeToFile(const string &str){
		ofstream outstream;
		outstream.open(str);
		char ch;
		int i,j;
	...
	template <template <typename K,class Alloc = allocator<K>> class T>
	char HexAdapter<T>::readFromFile(const string &str){
		string copy;
		int i,j,z=0;
	...
	template <template <typename K,class Alloc = allocator<K>> class T>
	void HexAdapter<T>::setSize(int a){
		size_board=a;
	}
	template <template <typename K,class Alloc = allocator<K>> class T>
	...
	template <template <typename K,class Alloc = allocator<K>> class T>
	Cell HexAdapter<T> :: operator()(int column,int row){
	...
	template <template <typename K,class Alloc = allocator<K>> class T>
	const string HexAdapter<T>::get_board()const{
		int i,j;
		string str;
		for(i=0;i<size_board;i++){
			for(j=0;j<size_board;j++){
